...
inline void discover_word_until_31_count(u8 *ret, u64 *current, u64 *virgin) {
  ...
  if (*current & *virgin) {
    ...
    // if (likely(*ret < 2)) {
      ...
      if ((cur[0]&vir[0]) && cur[0] >= 128) {
        if(vir[0] == 0xff){*ret = 2;}
        else if(*ret == 0){*ret = 1;}
        vir[0] &= ~cur[0];
      }
      if ((cur[1]&vir[1]) && cur[1] >= 128) {
        if(vir[1] == 0xff){*ret = 2;}
        else if(*ret == 0){*ret = 1;}
        vir[1] &= ~cur[1];
      }
      if ((cur[2]&vir[2]) && cur[2] >= 128) {
        if(vir[2] == 0xff){*ret = 2;}
        else if(*ret == 0){*ret = 1;}
        vir[2] &= ~cur[2];
      }
      if ((cur[3]&vir[3]) && cur[3] >= 128) {
        if(vir[3] == 0xff){*ret = 2;}
        else if(*ret == 0){*ret = 1;}
        vir[3] &= ~cur[3];
      }
      if ((cur[4]&vir[4]) && cur[4] >= 128) {
        if(vir[4] == 0xff){*ret = 2;}
        else if(*ret == 0){*ret = 1;}
        vir[4] &= ~cur[4];
      }
      if ((cur[5]&vir[5]) && cur[5] >= 128) {
        if(vir[5] == 0xff){*ret = 2;}
        else if(*ret == 0){*ret = 1;}
        vir[5] &= ~cur[5];
      }
      if ((cur[6]&vir[6]) && cur[6] >= 128) {
        if(vir[6] == 0xff){*ret = 2;}
        else if(*ret == 0){*ret = 1;}
        vir[6] &= ~cur[6];
      }
      if ((cur[7]&vir[7]) && cur[7] >= 128) {
       if(vir[7] == 0xff){*ret = 2;}
       else if(*ret == 0){*ret = 1;}
        vir[7] &= ~cur[7];
      }
