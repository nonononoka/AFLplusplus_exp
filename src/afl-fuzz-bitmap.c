...
inline u8 has_new_bits(afl_state_t *afl, u8 *virgin_map) {
  ...
  while (i--) {
    ...
    if (unlikely(*current)) discover_word(&ret, current, virgin, true);
...
inline u8 check_new_bits(afl_state_t *afl, u8 *virgin_map) {
  ...
  while (i--) {
    ...
    if (unlikely(*current)) discover_word(&ret, current, virgin, false);
...
static inline u8 check_new_bits_unclassified(afl_state_t *afl, u8 *virgin_map,
                                           bool *classified) {
  ...
  classify_counts(&afl->fsrv);
  *classified = true;
  return check_new_bits(afl, virgin_map);
}
...
static inline void calculate_new_bits_if_necessary_check_new_bits(afl_state_t *afl,
                                                   u8          *new_bits,
                                                   bool        *bits_counted,
                                                   bool        *classified) {

  if (*bits_counted) return;
  // countしか変化がないか否かを事前に見て、countしか変化がない場合は即returnする

  if (*classified) {
    u8 nb = check_new_bits(afl, afl->virgin_bits);
    if (nb != 2){return;}
    *new_bits = has_new_bits(afl, afl->virgin_bits); // 2だったらbitmapを更新しなきゃいけないのでもう一回これ

  } else {
    u8 nb = check_new_bits_unclassified(afl, afl->virgin_bits, classified);
    if (nb != 2){return;}
    *new_bits = has_new_bits(afl, afl->virgin_bits); // 同じく2だったらbitmapを更新しなきゃいけない
  }
...
u8 __attribute__((hot)) save_if_interesting(afl_state_t *afl, void *mem,
  ...
  if (likely(fault == afl->crash_mode)) {
    ...
    calculate_new_bits_if_necessary_check_new_bits(afl, &new_bits, &bits_counted, &classified);
