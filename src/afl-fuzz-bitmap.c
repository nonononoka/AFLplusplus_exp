...
inline u8 has_new_bits_only_new_edge(afl_state_t *afl, u8 *virgin_map) {
...
inline u8 has_new_bits_more_count_edge(afl_state_t *afl, u8 *virgin_map) {
  ...
  while (i--) {
    ...
    if (unlikely(*current)) discover_word_more_count_edge(&ret, current, virgin);
...
static inline u8 has_new_bits_unclassified_only_new_edge(afl_state_t *afl, u8 *virgin_map,
                                           bool *classified) {
...
static inline u8 has_new_bits_unclassified_more_count_edge(afl_state_t *afl, u8 *virgin_map,
                                           bool *classified) {
  ...
  classify_counts(&afl->fsrv);
  *classified = true;
  return has_new_bits_more_count_edge(afl, virgin_map);
...
static inline void calculate_new_bits_if_necessary_only_new_edge(afl_state_t *afl,
                                                   u8          *new_bits,
                                                   bool        *bits_counted,
                                                   bool        *classified) {
  ...
  if (*classified) {
    ...
    *new_bits = has_new_bits_only_new_edge(afl, afl->virgin_bits);

  } else {

    *new_bits = has_new_bits_unclassified_only_new_edge(afl, afl->virgin_bits, classified);
...
static inline void calculate_new_bits_if_necessary_more_count_edge(afl_state_t *afl,
                                                   u8          *new_bits,
                                                   bool        *bits_counted,
                                                   bool        *classified) {
  ...
  if (*classified) {
    ...
    *new_bits = has_new_bits_more_count_edge(afl, afl->virgin_bits);

  } else {

    *new_bits = has_new_bits_unclassified_more_count_edge(afl, afl->virgin_bits, classified);
...
u8 __attribute__((hot)) save_if_interesting(afl_state_t *afl, void *mem,
  ...
  if (likely(fault == afl->crash_mode)) {
    ...
    if (!afl->has_saturated_0_1){
      calculate_new_bits_if_necessary_only_new_edge(afl, &new_bits, &bits_counted, &classified);
    } else if(!afl->has_saturated_more){ // 0_1だとsaturateしたけど、数が多いやつはsaturateしていない
      calculate_new_bits_if_necessary_more_count_edge(afl, &new_bits, &bits_counted, &classified);
    }
    else{
      calculate_new_bits_if_necessary(afl, &new_bits, &bits_counted, &classified);
    }
    
    if ((afl->has_saturated_0_1 && !new_bits) || (!afl->has_saturated_0_1 && new_bits != 2)) {
