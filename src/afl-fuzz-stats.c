...
void count_coverage_by_area_when_saturated(afl_state_t *afl, u8 *mem) {
  afl->area_cnt = afl->fsrv.real_map_size / afl->area_divide_size + 1;
  u32 *ptr = (u32 *)mem;
    ...
    if (unlikely(afl->plot_prev_ed - afl->last_updated_execs >= ave_execs_per_s*900)){
      ...
      for (u32 i = 0; i < afl->area_cnt; i++) {
          ...
          if(afl->coverage_by_area[i] > afl->prev_coverage_by_area[i]){
            ...
            afl->prev_coverage_by_area[i] = afl->coverage_by_area[i];
          } else if(afl->progressing_count_by_area[i]){ // coverageが同じで、progressingが1以上だった場合は、マイナスにする
            afl->progressing_count_by_area[i]--;
          }
