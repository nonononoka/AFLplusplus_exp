  ...
  if (!afl->has_saturated_0_1){
    if (afl->plot_prev_ed - afl->last_updated_execs >= ave_execs_per_s*900){
      if (((double)(t_bytes - afl->prev_total_edge_found_in_15_minutes) / (double)afl->prev_total_edge_found_in_15_minutes) <= 0.01){
        afl->has_saturated_0_1 = 1;
        ACTF("has saturated!");
        afl->prev_total_corpus_count_in_15_minutes = afl->queued_items;
      }
      afl->prev_total_edge_found_in_15_minutes = t_bytes;
      afl->last_updated_execs = afl->plot_prev_ed; // 今までの実行回数
    }
  }
  else if(!afl->has_saturated_more){ // 0_1でsaturateしたら今の値よりカバレッジが大きくなるシードだけ加えていく
    // これはシードが埋まらなくなったらって感じでいいかも
    if (afl->plot_prev_ed - afl->last_updated_execs >= ave_execs_per_s*900){
      if (((double)(afl->queued_items - afl->prev_total_corpus_count_in_15_minutes) / (double)afl->prev_total_corpus_count_in_15_minutes) <= 0.1){
        afl->has_saturated_more = 1;
        ACTF("has saturated again!");
      }
      afl->prev_total_corpus_count_in_15_minutes = afl->queued_items;
      afl->last_updated_execs = afl->plot_prev_ed; // 今までの実行回数
    }
